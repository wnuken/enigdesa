...
class ViviendaAcms extends MX_Controller {
	...
	//hhchavezv - Valido si est� en seccion 2, para que seleccione en switch de control de 5 a 8
		...
		if ($this->idSeccion =="C1"){
			...
			$arrSA[0]['PAGINA_CONTROL']=9;//$arrSA["1"]["PAG_SECCION3"]+8;
		}else if ($this->idSeccion =="C4"){
			$arrSA[0]['PAGINA_CONTROL']=10;$arrSA["1"]["PAG_SECCION3"]+9;
		}
 ...
 /* OK - se quita 
           ...
           // switch ($arrSA["0"]["PAG_SECCION3"]) { //orig
			...
			switch ($arrSA[0]['PAGINA_CONTROL']) {
				...
				case 9: //echo "mostrarFormServiciosPublicos";
					...
					//Al inicio ID_ESTADO_SEC y PAG_SECCION3 estan en 0 ; liego de la primera validacion queda 1 - 1 ;y al ternimar queda 2- 1
					// PERO Segun lo hablado inicia en 1 1; luego de priemra valid queda en 1 2; al terminar 2-2
                    break;
				case 10:
    ...
    /*private function mostrarListaObtencion() {
        ...
        $data["js_dir"] = base_url('js/' . $this->module . '/' . $this->submodule . '/archivo.js');
        $data["view"] = $this->submodule . '/view2';
        $this->load->view("layout", $data);
    }*/
     ...
     * @since 
     */
    private function mostrarFormServiciosPublicos() {
        //Llamar metodo de otro controlador
		require_once("ViviendaServicios.php");
		$controller = new ViviendaServicios();
		$controller->index();
    }
     ...
     */
     ...
     //  pr($arrSA);
        ...
        $this->idSeccion = $arrSA[1]['ID_SECCION3'];
			...
			if($arrSA[1]['ID_SECCION3'] =="C1" || $arrSA[1]['ID_SECCION3'] =="C2" )
			{	
	...
	// echo $fin ."&&". sizeof($arrSA) ."> 2 &&". $arrSA[2]['ID_ESTADO_SEC'] ."==  0 &&". $arrSA[2]['PAG_SECCION3'];
			...
			}else if($arrSA[1]['ID_SECCION3'] =="C3")	{ 
				...
				} // El controlador ViviendaServicios actualiza controles automaticamente ( guarda preg1 en ENIG_FORM_GMF_VARIABLES, var P10272S5; y resto en tabla ENIG_FORM_GMF_SERVICIOS
				else if($arrSA[1]['ID_ESTADO_SEC'] ==  2 && $arrSA[1]['PAG_SECCION3'] ==  3){// ya termino C3
									
					// Inicializo seccion C4
					$this->Modgmfh->ejecutar_update('ENIG_ADMIN_GMF_CONTROL', array( "ID_ESTADO_SEC" => 1, "PAG_SECCION3" => 1, "FECHA_INI_SEC" => $fechaactual), array( "ID_FORMULARIO" => $id_formulario, "ID_SECCION3" => "C4"));
					$arrSA[2]['ID_ESTADO_SEC'] = 1;
					//echo "sigue 2 sec";           
					$arrSA[2]['PAG_SECCION3'] = 1;
				}
			...
			}else if($arrSA[1]['ID_SECCION3'] =="C4"){	
