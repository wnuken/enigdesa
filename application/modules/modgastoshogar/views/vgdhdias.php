...
?>
	...
	<div class="row text-center">
		<div class="col-sm-12"><button class='btn btn-success' id='ENV_14DIA14' onclick="$('#DIA_confirm').dialog('open');">Guardar y Finalizar <span class='glyphicon glyphicon-chevron-right' aria-hidden='true' title='Continuar'></span></button></div>
	</div>
<?
	}
	else {
	...
	$("#DIA_confirm").dialog({
		...
		buttons: {
			Aceptar: function() {
				$.ajax({
					type: 'POST',
					url: '<?php echo site_url("modgastoshogar/Gastoshog/findia") ."/";?>'+ $('#ID_SECCION').val(),
					cache: false,
					contentType: "application/x-www-form-urlencoded; charset=UTF-8",
					success: function (respuesta) {
						if ($('#ID_SECCION').val() == "14DIA14")
							$('#ENV_14DIA14').verificar_enviar('14DIA14','<? echo site_url("modgastoshogar/Gastoshog/guardar/14DIA14");?>', regla_d14);
						else
							window.location = '<? echo site_url("modgastoshogar/Gastoshog/index") ."/". $id_persona;?>';
						$('#mensaje'+ $('#ID_SECCION').val()).html('<div class="alert alert-success" role="alert"><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span>'+respuesta+'<span id="btn_seguir"><span></div>');
					},
					error: function (respuesta) {
...
?>
		$.ajax({
			type: "POST",
			url: "<?php echo site_url("modgastoshogar/Gastoshog/articulos") ."/"; ?>"+ $('#_DIA_ART').val(),
			cache: false,
			success: function (response) {
				...
				$.each(datos, function (i, item) {
					...
					trHTML += "<tr><td>"+ item.NH_CGDU_P1_1 +"</td><td>" + item.P10250S1C2M + "</td><td>" + item.NH_CGDU_P2 + 
						...
						"<td align='center'><a onclick=\"cargaformArt('<?php echo base_url("modgastoshogar/Gastoshog/articulo")."/";?>" + 
					   item.ID_ARTICULO +"');\"><img src='<?php echo base_url("images/form_icon-editar.png"); ?>'/></a></td>" +
					   "<td align='center'><a onclick=\"eliminaformArt('<?php echo base_url("modgastoshogar/Gastoshog/noarticulo")."/";?>" + 
...
?>		$.ajax({
			type: "POST",
			url: "<?php echo site_url("modgastoshogar/Gastoshog/comidas") ."/";?>"+ $('#_DIA_COM').val(),
			cache: false,
			success: function (response) {
				var subtot = 0;
				var datos = $.parseJSON($.trim(response));
				...
				$.each(datos, function (i, item) {
					...
					trHTML += "<tr><td>"+ item.NH_CGDUCFH_P1 +"</td><td>" + item.NH_CGDUCFH_P1_1 + "</td><td>" + item.NH_CGDUCFH_P2 + 
						...
						"<td align='center'><a onclick=\"cargaformCom('<?php echo base_url("modgastoshogar/Gastoshog/comida")."/";?>" + 
					   item.ID_COMIDA +"');\"><img src='<?php echo base_url("images/form_icon-editar.png"); ?>'/></a></td>" +
					   "<td align='center'><a onclick=\"eliminaformCom('<?php echo base_url("modgastoshogar/Gastoshog/nocomida")."/";?>" + 
