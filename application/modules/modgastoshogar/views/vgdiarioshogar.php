	...
	<div class="row">
		<div class="col-sm-6 text-right"><button type='button' id='btn_articulos' onclick="recargatablaArt();$('#GDH_ARTICULOS').dialog('open');">A&ntilde;adir</button></div>
		<div class="col-sm-1"></div><div class="col-sm-5" id="mensaje_ARTICULOS"></div>
	</div>
	<hr />
	...
	<div class="row">
		<div class="col-sm-6 text-right"><button type='button' id='btn_comidas' onclick="recargatablaCom();$('#GDH_COMIDAS').dialog('open');">A&ntilde;adir</button></div>
		<div class="col-sm-1"></div><div class="col-sm-5" id="mensaje_COMIDAS"></div>
	</div>
	<br/>
<?
	if ($secc == "14DIA14") {
...
?>
	...
	<div class="row text-center">
		<div class="col-sm-12"><button class='btn btn-success' id='ENV_14DIA14'>Guardar y Finalizar <span class='glyphicon glyphicon-chevron-right' aria-hidden='true' title='Continuar'></span></button></div>
	</div>
<?
	}
	else {
...
?>
    ...
    function cargaformCom(url) {
		$('#mensaje_COMIDAS').html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Un momento por favor..</div>');
		$.ajax({
			type: 'POST',
			url: url,
			contentType: "application/x-www-form-urlencoded; charset=UTF-8",
...
?>				$('#mensaje_COMIDAS').html('');
    ...
    function eliminaformCom(url) {
        $.ajax({
            ...
            cache: false,
            success: function (respuesta) {
				$("#mensaje_COMIDAS").html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Registro eliminado</div>');
                recargatablaCom();
				$("#mensaje_COMIDAS").html('');
			}
        });
			...
			"Guardar": function() {
				...
				if (estado) {
					...
					$.ajax({
						...
						data: args,
						beforeSend: function (objeto) {
							$('#mensaje_COMIDAS').html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Enviando m&oacute;dulo</div>');
						},
						success: function (respuesta) {
							$('#mensaje_COMIDAS').html('<div class="alert alert-success" role="alert"><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> '+ respuesta +'<span id="btn_seguir"><span></div>');
							recargatablaCom();
						},
						error: function (respuesta) {
							$('#mensaje_COMIDAS').html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> Error guardando m&oacute;dulo</div>');
						}
					});
					$(this).attr('disabled', false);
					$( this ).dialog( "close" );
				} else {
					$('#mensaje_COMIDAS').html('<div class="alert alert-warning" role="alert"><span class="glyphicon glyphicon-question-sign" aria-hidden="true"></span> Hay campos que requieren correcciones, por favor verifique e intente nuevamente</div>');
				}
			}
		}
	});
