	<div class="row">
		...
		<div class="col-sm-4" id="mensaje_<?echo $seccion?>"></div>
	</div>
	...
	<div class="row">
		<div class="col-sm-12 text-center"><button type='button' onclick="recargatablaArt_<?echo $seccion;?>();$('#GDH_ARTICULOS_<?echo $seccion;?>').dialog('open');">A&ntilde;adir</button></div>
	</div>
	<hr />
	...
	<div class="row">
		<div class="col-sm-12 text-center"><button type='button' id='btn_comidas' onclick="recargatablaCom_<?echo $seccion;?>();$('#GDH_COMIDAS_<?echo $seccion;?>').dialog('open');">A&ntilde;adir</button></div>
	</div>
	<br/>
<?
	if ($seccion == "14DIA14") {
...
?>
	<div id="GDH_ARTICULOS_<?echo $seccion;?>" title="Gastos de Hoy">
		<form id="form_ARTICULOS_<?echo $seccion;?>" name="form_ARTICULOS_<?echo $seccion;?>" class="form-horizontal" role="form">
			<input type="hidden" name="ID_FORMULARIO" id="ID_FORMULARIO" value="<?echo $id_formulario; ?>" />
			...
			<input type="hidden" id="_RMED" value='SI'/>
...
?>
	...
	<div id="GDH_COMIDAS_<?echo $seccion;?>" title="Comidas preparadas fuera del hogar">
		<form id="form_COMIDAS_<?echo $seccion;?>" name="form_COMIDAS_<?echo $seccion;?>" class="form-horizontal" role="form">
			<input type="hidden" name="ID_FORMULARIO" id="ID_FORMULARIO" value="<?echo $id_formulario; ?>" />
...
?>
    function cargaformArt_<?echo $seccion;?>(url) {
		$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Un momento por favor..</div>');
		$.ajax({
			type: 'POST',
			url: url,
			...
			success: function (respuesta) {
				$('#form_ARTICULOS_<?echo $seccion;?>').find('input, select').attr('disabled',false);
...
?>
				$('#mensaje_<?echo $seccion;?>').html('');
			}
		});
		$("#GDH_ARTICULOS_<?echo $seccion;?>").dialog('open');
    }
    function eliminaformArt_<?echo $seccion;?>(url) {
        $.ajax({
            type: 'POST',
            url: url,
            contentType: "application/x-www-form-urlencoded; charset=UTF-8",
            cache: false,
            success: function (respuesta) {
				$("#mensaje_<?echo $seccion;?>").html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Articulo eliminado</div>');
                recargatablaArt_<?echo $seccion;?>();
				$("#mensaje_<?echo $seccion;?>").html('');
			}
        });
    }
    function recargatablaArt_<?echo $seccion;?>() {
...
?>
		$.ajax({
			type: "POST",
			url: "<?php echo site_url("modgastoshogar/Gastoshog/articulos"); ?>",
			cache: false,
			success: function (response) {
				...
				$.each(datos, function (i, item) {
					...
					trHTML += "<tr><td>"+ item.NH_CGDU_P1_1 +"</td><td>" + item.P10250S1C2M + "</td><td>" + item.NH_CGDU_P2 + 
						...
						"<td align='center'><a onclick=\"cargaformArt_<?echo $seccion;?>('<?php echo base_url("modgastoshogar/Gastoshog/articulo")."/";?>" + 
					   item.ID_ARTICULO +"');\"><img src='<?php echo base_url("images/form_icon-editar.png"); ?>'/></a></td>" +
					   "<td align='center'><a onclick=\"eliminaformArt_<?echo $seccion;?>('<?php echo base_url("modgastoshogar/Gastoshog/noarticulo")."/";?>" + 
	...
	//////
    function cargaformCom_<?echo $seccion;?>(url) {
		$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Un momento por favor..</div>');
		$.ajax({
			type: 'POST',
			url: url,
			...
			cache: false,
			success: function (respuesta) {
				$('#form_COMIDAS_<?echo $seccion;?>').find('input, select').attr('disabled',false);
...
?>				$('#mensaje_<?echo $seccion;?>').html('');
		...
		$("#GDH_COMIDAS_<?echo $seccion;?>").dialog('open');
    }
    function eliminaformCom_<?echo $seccion;?>(url) {
        $.ajax({
            type: 'POST',
            url: url,
            contentType: "application/x-www-form-urlencoded; charset=UTF-8",
            cache: false,
            success: function (respuesta) {
				$("#mensaje_<?echo $seccion;?>").html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Comida eliminada</div>');
                recargatablaCom_<?echo $seccion;?>();
				$("#mensaje_<?echo $seccion;?>").html('');
			}
        });
    }
    function recargatablaCom_<?echo $seccion;?>() {
...
?>		$.ajax({
			type: "POST",
			url: "<?php echo site_url("modgastoshogar/Gastoshog/comidas"); ?>",
			cache: false,
			success: function (response) {
				var subtot = 0;
				var datos = $.parseJSON($.trim(response));
				...
				$.each(datos, function (i, item) {
					...
					trHTML += "<tr><td>"+ item.NH_CGDUCFH_P1 +"</td><td>" + item.NH_CGDUCFH_P1_1 + "</td><td>" + item.NH_CGDUCFH_P2 + 
						...
						"<td align='center'><a onclick=\"cargaformCom_<?echo $seccion;?>('<?php echo base_url("modgastoshogar/Gastoshog/comida")."/";?>" + 
					   item.ID_COMIDA +"');\"><img src='<?php echo base_url("images/form_icon-editar.png"); ?>'/></a></td>" +
					   "<td align='center'><a onclick=\"eliminaformCom_<?echo $seccion;?>('<?php echo base_url("modgastoshogar/Gastoshog/nocomida")."/";?>" + 
...
$(function() {
	$('#_NH_CGDU_P1_1').autocomplete({
		appendTo: "#GDH_ARTICULOS_<?echo $seccion;?>",
	...
	$("#GDH_ARTICULOS_<?echo $seccion;?>").dialog({
		...
		buttons: {
			"Guardar": function() {
				...
				if (estado) {
					var myf = $('#form_ARTICULOS_<?echo $seccion;?>');
					...
					$.ajax({
						...
						beforeSend: function (objeto) {
							$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-info" role="alert"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Enviando m&oacute;dulo</div>');
						},
						success: function (respuesta) {
							$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-success" role="alert"><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> '+ respuesta +'<span id="btn_seguir"><span></div>');
							recargatablaArt_<?echo $seccion;?>();
						},
						error: function (respuesta) {
							$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> Error guardando m&oacute;dulo</div>');
						}
					});
					$(this).attr('disabled', false);
					$( this ).dialog( "close" );
				} else {
					$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-warning" role="alert"><span class="glyphicon glyphicon-question-sign" aria-hidden="true"></span> Hay campos que requieren correcciones, por favor verifique e intente nuevamente</div>');
				}
			}
		}
	});
	$("#GDH_COMIDAS_<?echo $seccion;?>").dialog({
		open: function(event, ui) {
			$("input").blur();
		},
		autoOpen: false,
		...
		buttons: {
			"Guardar": function() {
				...
				if (estado) {
					var myf = $('#form_COMIDAS_<?echo $seccion;?>');
					var args = myf.serialize().replace(/(%0D%0A|%0D|%0A|%22|%5C|')/g, " ");
					//$(this).attr('disabled', true);
					$.ajax({
						type: 'POST',
						url: '<?php echo site_url("modgastoshogar/Gastoshog/guardar/COMIDAS");?>',
						cache: false,
						contentType: "application/x-www-form-urlencoded; charset=UTF-8",
						data: args,
						beforeSend: function (objeto) {
...
?>
					...
					error: function (respuesta) {
						$('#mensaje_<?echo $seccion;?>').html('<div class="alert alert-danger" role="alert"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> Error guardando Dia.</div>');
					}
				});
				$( this ).dialog( "close" );
			},
