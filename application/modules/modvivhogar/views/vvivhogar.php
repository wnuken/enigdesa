...
<?php
	...
	if ($secc['SIGUIENTE'] == 'SI') {
		...
		elseif ($secc['ACCION'] == 'FINALIZAR')
			echo "	<button class='btn btn-success' id='ENV_". $secc['ID_SECCION'] .'_'. $secc['PAGINA'] ."'>Guardar y Finalizar <span class='glyphicon glyphicon-chevron-right' aria-hidden='true' title='Finalizar'></span></button>\n";
	}
?>
		</div>
<script>
...
///// casos especiales
	...
	$('#P5100S1').blur(function() {
		...
		var val2 = 0;
		var val3 = 0;
		if (!($("#P5100S1").val() == 98 || $("#P5100S1").val() == 99)) {
			val1 = parseInt($("#P5100S1").val());
		}
		if (!($("#P5100S2").val() == 98 || $("#P5100S2").val() == 99)) {
			val2 = parseInt($("#P5100S2").val());
		}
		if (!($("#P5100S3").val() == 98 || $("#P5100S3").val() == 99)) {
			val3 = parseInt($("#P5100S3").val());
		}
		$("#P5100S4").val(val1 + val2 + val3);
	});
...
/*	$('#P5103').blur(function () {
	...
	$('[name=P5160]').dependencias('P5160', 'GRP_P5161S1', [1]);
	$('[name=P5090]').dependencias('P5090', 'GRP_P1644', [1]);
