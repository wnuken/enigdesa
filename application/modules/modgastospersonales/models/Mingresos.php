...
class Mingresos extends CI_Model {
	...
	public function listadoPersonasRegistro($id_formulario) {
		...
		AS FIN_CARAC, (SELECT FINALIZADO FROM ENIG_ADMIN_AVANCESPERSONAS A 
		WHERE P.ID_FORMULARIO=A.ID_FORMULARIO AND P.ID_PERSONA=A.ID_PERSONA AND ID_SECCION='13NEGOMIN' AND PAGINA='1') 
		AS FIN_INGRE FROM ENIG_FORM_PERSONAS P WHERE P.ID_FORMULARIO='". $id_formulario ."' AND P6040 >= 3 ORDER BY P521A,P521C";
...
/*	public function listarSeccionesxCapitulo($id_formulario, $id_persona, $capitulo) {
	...
	public function variablesDependientes($seccion, $pagina, $id_formulario, $id_persona) {
		$vars = array();
		$depen = array();
		// obtener variables dependientes
		$sql3 = "SELECT VAR_ORIGEN, VO.TIPO_CAMPO AS VO_TIPOCAMPO, VO.ID_SECCION as VO_SECCION, VO.PAGINA AS VO_PAGINA, VAR_DESTINO, VD.TIPO_CAMPO AS VD_TIPOCAMPO, 
		VD.ID_SECCION AS VD_SECCION, VD.PAGINA AS VD_PAGINA, DEPENDENCIA FROM ENIG_ADMIN_DEPENDENCIAS D,ENIG_ADMIN_VARIABLES VO, ENIG_ADMIN_VARIABLES VD 
		WHERE VO.ID_VARIABLE=VAR_ORIGEN AND VD.ID_VARIABLE=VAR_DESTINO AND VD.ID_SECCION='$seccion' AND VD.PAGINA='$pagina'";
		...
		if ($query3->num_rows() > 0) {
			...
			foreach ($query3->result() as $row) {
				...
				if ($row->VO_SECCION . $row->VO_PAGINA != $row->VD_SECCION . $row->VD_PAGINA) {
					$depen[$i]['VO'] = $row->VAR_ORIGEN;
					$depen[$i]['VO_SP'] = $row->VO_SECCION . $row->VO_PAGINA;
					$depen[$i]['VD'] = $row->VAR_DESTINO;
					$depen[$i]['VD_SP'] = $row->VD_SECCION . $row->VD_PAGINA;
					$depen[$i]['DEPENDENCIA'] = $row->DEPENDENCIA;
					$i++;
				}
			}
			$vals = $this->listarValores($volis, $id_formulario, $id_persona);
			foreach ($depen as $v) {
				$d = explode(',', str_replace("[", "", str_replace("]", "", $v['DEPENDENCIA'])));
				if (in_array($vals[$v['VO']], $d))
					$vars[$v['VD']] = $v['VD'];
			}
		}
		return $vars;
